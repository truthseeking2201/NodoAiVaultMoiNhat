...
import { Rankings, RewardEntry } from "@/types/leaderboards.types";
...
const Banner = () => {
  ...
  const prize = useMemo(() => {
    if (data) {
      const total1 = sumRewards(data?.tvl_config_rewards?.rankings);
    }
    // TODO
  ...
  function sumRewards(rankings: Rankings) {
    return Object.values(rankings)?.reduce<RewardEntry>(
