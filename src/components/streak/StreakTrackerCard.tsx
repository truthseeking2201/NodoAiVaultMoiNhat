...
import { CardHeaderBar, HeaderPill } from "@/components/ui/card-header-bar";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
import { ArrowRight, Check, Flame, Gift } from "lucide-react";
...
export function StreakTrackerCard({
  ...
  const statusTone = todayDone ? "success" : "neutral";
  const statusLabel = todayDone
    ? "Today completed"
    : `Resets in ${formatTimeLeft(resetInMs)}`;
  const statusPill = <HeaderPill tone={statusTone}>{statusLabel}</HeaderPill>;
  const headerRight = <span aria-live="polite">{statusPill}</span>;
  const titleId = showTitle ? "streak-tracker-heading" : undefined;

  return (
    <Card
      className={cn(
        "glass-card overflow-hidden rounded-xl border border-white/10 bg-white/5 text-white shadow-[0_2px_20px_rgba(0,0,0,0.25)] backdrop-blur-sm",
        className
      )}
    ...
    >
      ...
      <div className={cn("p-6", !showTitle && "pt-4")}>
        ...
        <TooltipProvider delayDuration={100}>
          <div className="relative mb-4">
            ...
            {milestones.map((milestone) => {
              ...
              const isNext =
                milestone.days === nextMilestone.days && !isAchieved;
                  ...
                  <TooltipTrigger asChild>
                    <div
                      className={cn(
                        ...
                        isNext &&
                          "!border-amber-400/40 bg-amber-400/20 backdrop-blur"
                      )}
                      style={{ left }}
